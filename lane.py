...
class Lane(SimulationObject):
    ...
    def fix_vehicle_position(self, vehicle):
        """Fix the position of the given vehicle, by placing it back on it's path.

        Fixing is needed whenever a vehicle 'reaches' a target by overshooting it.
        A new target is selected after fixing the position.
        If the fixed position would be behind the last lane node, the vehicle is removed.
        If the vehicle is on a trajectory, follow the trajectory before following the lane.

        :param vehicle: vehicle to fix
        :return: None
        """
    ...
    def remove_vehicle(self, vehicle):
        """Remove the given vehicle from this lane.
        ...
        if vehicle in self.vehicles:
            ...
            print("Attempted to remove vehicle from wrong lane.")
    ...
    def projected_velocity(self, vehicle):
        """Project the velocity ot the given vehicle on the lane.

        :param vehicle: vehicle of which to project the velocity
    ...
    def approaching_velocity(self, v1, v2):
        """Calculate the signed approaching velocity of a vehicle in relation to another vehicle.
